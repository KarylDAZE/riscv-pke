...
void load_bincode_from_host_elf(process *p)
{
    ...
    elf_sect_header shstrtabHeader, tmpHeader, debugLineHeader;
    debug_header debugHdr;
    int i;
    elf_fpread(&elfloader, &shstrtabHeader, sizeof(elf_sect_header), shstrtabHdrOff);
    char shstrtab[shstrtabHeader.size];
    elf_fpread(&elfloader, &shstrtab, sizeof(shstrtab), shstrtabHeader.addr + shstrtabHeader.offset);
    ...
    for (i = 0; i < elfloader.ehdr.shnum; i++)
    {
        ...
        if (strcmp(shstrtab + tmpHeader.name, ".debug_line") == 0)
