                                   ...
                                   char *miss_name)
{
  ...
  if (path_copy[0] == '.')
  {
    ...
    do_rcwd(absPath); // current work directory
    if (path_copy[1] == '/')
    {
      if (absPath[1] != 0) // 不是根目录
        strcat(absPath, path_copy + 1);
      else
        strcat(absPath, path_copy + 2);
    }
    else if (path_copy[1] == '.' && (path_copy[2] == '/' || path_copy[2] == 0))
    {
      for (int i = strlen(absPath); i >= 0; i--)
      {
        if (absPath[i] == '/')
        {
          if (i) // 不是根目录
